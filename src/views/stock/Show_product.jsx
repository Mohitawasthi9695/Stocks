...
const Show_product = () => {
  ...
  const handleDownloadExcel = (row) => {
    // Destructure only required keys by excluding id, invoice_id, product_id
    const { id, invoice_id, product_id, ...filteredRow } = row;
  
    const worksheet = XLSX.utils.json_to_sheet([filteredRow]); // Use filtered row
  ...
  const exportToCSV = () => {
    ...
    // 👉 Define headers in the order you want
    const csvHeaders = [
      'Sr No',
      'Date',
      'Invoice No',
    ...
    const csvData = filteredProducts.map((row, index) => ({
      ...
      'Product Category': row.product_category_name ?? 'N/A',
      'Shade No': row.shadeNo ?? 'N/A',
      'Pur. Shade No': row.purchase_shade_no ?? 'N/A',
      'Length': `${Number(row.length).toFixed(2)} ${row.length_unit}`,
      'Width': `${Number(row.width).toFixed(2)} ${row.width_unit}`,
    ...
    const csvString = Papa.unparse({
      fields: csvHeaders,
      data: csvData.map(row => csvHeaders.map(header => row[header]))
    });
