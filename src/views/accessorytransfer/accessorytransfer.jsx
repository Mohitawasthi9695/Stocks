...
import { FaUser, FaShare, FaUserPlus, FaTrash, FaPlus } from 'react-icons/fa';
  ...
  useEffect(() => {
    const fetchReceiverData = async () => {
      try {
        ...
        console.log(response.data.data);
        const loggedInUserId = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')).id : null;
        const filteredData = response.data.data.filter(subSupervisor => subSupervisor.id != loggedInUserId);
        setSub_supervisor(filteredData);
      } catch (err) {
        console.log(err);
      }
    };
      ...
      <Row className="justify-content-center">
        <Col md={12} lg={12}>
          <Card className="shadow-lg border-0" style={{ borderRadius: '15px' }}>
            ...
            >
              <FaShare size={40} className="me-3" />
              <h2 className="m-0 text-white">Transfer To GoDown</h2>
            </div>
            <Card.Body className="p-5">
              <Form onSubmit={handleSubmit}>
                ...
                >
                  <FaShare className="me-2" /> Transfer
                </Button>
              </Form>
            </Card.Body>
          </Card>
