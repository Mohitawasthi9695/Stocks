...
//     // Convert width and length to meters if needed
//     if (row.width_unit && conversionFactors[row.width_unit]) {
//       width *= conversionFactors[row.width_unit];
//     }
//     if (row.length_unit && conversionFactors[row.length_unit]) {
//       length *= conversionFactors[row.length_unit];
//     }

//     // Ensure no NaN values
//     if (isNaN(width) || isNaN(length) || isNaN(rate) || isNaN(out_pcs)) {
//       return "0.00"; // Fallback to 0 if any issue
//     }

//     // Calculate total amount
//     return (rate * width * length * out_pcs).toFixed(2);
//   };
...
const Invoice_out = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ Validate that every product has a rate and amount
  ...
  const handleInputChange = (id, field, value) => {
    const updatedRows = selectedRows.map((row) => {
      if (row.godown_id === id) {
        ...
        // ✅ Ensure amount updates whenever rate or dimensions change
        if (["rate", "width", "length", "out_pcs", "width_unit", "length_unit"].includes(field)) {
          updatedRow.amount = calculateAmount(updatedRow);
        }
  ...
  const calculateAmount = (row) => {
    ...
    // ✅ Correct unit conversion (Convert cm/inch to meters)
