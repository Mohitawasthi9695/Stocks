...
const ShowProduct = () => {
    ...
    const fetchStocksData = async () => {
      try {
        ...
        const flattenedData = response.data.data.stock_out_details.map((detail, index) => ({
          ...
          length: detail.length_unit === 'inches' ? parseFloat(detail.length) * 39.3701 : parseFloat(detail.length),
          width: detail.width_unit === 'inches' ? parseFloat(detail.width) * 39.3701 : parseFloat(detail.width),
          unit: detail.length_unit || 'meter',
          qty: detail.out_pcs || 'N/A',
          amount: detail.amount || 'N/A',
          status: detail.status === 1 ? 'Approved' : 'Pending',
  ...
  const columns = [
    ...
    {
      name: 'Status',
      selector: (row) => (row.status === 0 ? 'pending' : 'approve'),
      sortable: true,
      cell: (row) => (
        <span
          style={{
            backgroundColor: row.status === 0 ? 'red' : 'green', 
            color: 'white', 
            padding: '5px 10px',
            borderRadius: '5px',
            fontWeight: 'bold',
