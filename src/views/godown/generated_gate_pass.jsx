...
const Index = () => {
  ...
  const downloadExcel = (row) => {
    ...
    console.log("Full Invoice Data:", fullInvoice); // Debugging
  
    if (!fullInvoice || !fullInvoice.all_stocks) {
      console.error('Godown data not found for this row:', row);
      toast.error('Data not found for the selected row!');
      return;
    }
    ...
    const extractedData = fullInvoice.all_stocks.map((godown) => ({
      ...
      ProductType: godown.product_type,
      LotNo: godown.lot_no,
      ProductName: godown.product.name || 'N/A', // Handle missing product data
      ShadeNo: godown.products.shadeNo || 'N/A',
      StockCode: godown.stock_code || 'N/A',
      Width: godown.width || 'N/A',
      Length: godown.length || 'N/A',
      Pcs: godown.pcs || 0,
      Quantity: godown.quantity || 0,
      Supervisor: fullInvoice.warehouse_supervisor?.name || 'N/A',
    }));
  
    console.log("Extracted Data for Excel:", extractedData); // Debugging
  
    if (extractedData.length === 0) {
      toast.error('No stock data available for this invoice!');
      return;
    }
    ...
    saveAs(blob, `GatePass_${fullInvoice.gate_pass_no}.xlsx`);
  ...
  return (
    <div className="container-fluid pt-4" style={{ border: '3px dashed #14ab7f', borderRadius: '8px', background: '#ff9d0014' }}>
      ...
      {invoiceAllDetails && selectedInvoice && (
        <TheramalStockGatePass
          show={showPdfModalThermal}
          onHide={() => setShowPdfModalThermal(false)}
