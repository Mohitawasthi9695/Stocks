...
const ShowProduct = () => {
  ...
  useEffect(() => {
    const fetchStocksData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/godownstock/cutstock/${id}`, {
          headers: {
        ...
        const productsWithArea = response.data.data.map((product) => {
          ...
          return {
            ...
            available_length: (parseFloat(product.length) - parseFloat(product.out_length)).toFixed(3) // Compute Available Length
          };
        });
  ...
  const columns = [
    ...
    { name: 'Sub Stock Code', selector: (row) => row.sub_stock_code, sortable: true },
    { name: 'Lot No', selector: (row) => row.lot_no, sortable: true },
    { name: 'GatePass No', selector: (row) => row.gate_pass_no, sortable: true },
    ...
    { name: 'Available Length', selector: (row) => `${row.available_length} ${row.length_unit}`, sortable: true },
