...
const Show_product = () => {
    ...
    useEffect(() => {
        const fetchProductData = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/cutaccessory`, {
                    params: {
    ...
    const handleDelete = async (productId) => {
        ...
        try {
            const response = await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/cutaccessory/${productId}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            });
